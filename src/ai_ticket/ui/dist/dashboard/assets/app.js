import React, { useEffect, useMemo, useState } from "https://esm.sh/react@18.2.0";
import MetricsPanel from "./components/MetricsPanel.js";
import StatusGrid from "./components/StatusGrid.js";
import HueSlider from "./components/HueSlider.js";
import HistoryExplorer from "./components/HistoryExplorer.js";
import AdminPanel from "./components/AdminPanel.js";
import ShortcutOverlay from "./components/ShortcutOverlay.js";
import { useMetricsStream } from "./hooks/useMetricsStream.js";
import { useKeyboardShortcuts } from "./hooks/useKeyboardShortcuts.js";
import { useLocalStorageState } from "./hooks/useLocalStorageState.js";

const HISTORY_LIMIT = 60;
const logoUrl = "./logo.svg";

const App = () => {
  const [hue, setHue] = useLocalStorageState("dashboard-hue", 210);
  const [theme, setTheme] = useLocalStorageState("dashboard-theme", "dark");
  const [adminOpen, setAdminOpen] = useLocalStorageState("dashboard-admin-open", false);
  const [followLive, setFollowLive] = useLocalStorageState("dashboard-follow-live", true);
  const [streamPaused, setStreamPaused] = useLocalStorageState("dashboard-stream-paused", false);
  const [reducedMotion, setReducedMotion] = useLocalStorageState("dashboard-reduced-motion", false);
  const [selectedTimestamp, setSelectedTimestamp] = useLocalStorageState("dashboard-history-selection", null);
  const [shortcutOverlayOpen, setShortcutOverlayOpen] = useState(false);

  const { metrics, refresh } = useMetricsStream({ paused: streamPaused });
  const [history, setHistory] = useState([]);

  useEffect(() => {
    setHistory((previous) => {
      if (previous.length > 0 && previous[previous.length - 1].timestamp === metrics.timestamp) {
        return previous;
      }
      const next = [...previous, metrics];
      if (next.length > HISTORY_LIMIT) {
        return next.slice(next.length - HISTORY_LIMIT);
      }
      return next;
    });
  }, [metrics]);

  useEffect(() => {
    document.documentElement.style.setProperty("--accent-hue", String(hue));
  }, [hue]);

  useEffect(() => {
    document.documentElement.setAttribute("data-theme", theme);
  }, [theme]);

  useEffect(() => {
    document.documentElement.setAttribute("data-reduced-motion", reducedMotion ? "true" : "false");
  }, [reducedMotion]);

  useEffect(() => {
    if (followLive) {
      setSelectedTimestamp(null);
    }
  }, [followLive, metrics.timestamp, setSelectedTimestamp]);

  const activeSnapshot = useMemo(() => {
    if (!selectedTimestamp) {
      return metrics;
    }
    const match = history.find((snapshot) => snapshot.timestamp === selectedTimestamp);
    return match || metrics;
  }, [selectedTimestamp, history, metrics]);

  const isLive = !selectedTimestamp || selectedTimestamp === metrics.timestamp;

  const selectTimestamp = (timestamp) => {
    if (timestamp === null) {
      setSelectedTimestamp(null);
      return;
    }
    setFollowLive(false);
    setSelectedTimestamp(timestamp);
  };

  const stepHistory = (delta) => {
    if (history.length === 0) {
      return;
    }
    const currentIndex = isLive ? history.length - 1 : history.findIndex((snapshot) => snapshot.timestamp === selectedTimestamp);
    const safeIndex = currentIndex === -1 ? history.length - 1 : currentIndex;
    const targetIndex = Math.min(history.length - 1, Math.max(0, safeIndex + delta));
    const target = history[targetIndex];
    if (!target) {
      return;
    }
    if (targetIndex === history.length - 1) {
      setSelectedTimestamp(null);
      setFollowLive(true);
    } else {
      setFollowLive(false);
      setSelectedTimestamp(target.timestamp);
    }
  };

  const jumpToLive = () => {
    setFollowLive(true);
    setSelectedTimestamp(null);
  };

  const shortcutSections = useMemo(
    () => [
      {
        title: "Navigation",
        shortcuts: [
          { combo: "Shift + /", description: "Toggle shortcut overlay" },
          { combo: "Ctrl + .", description: "Toggle admin panel" },
          { combo: "Escape", description: "Close overlays" },
        ],
      },
      {
        title: "Realtime",
        shortcuts: [
          { combo: "R", description: "Refresh metrics" },
          { combo: "H", description: "Shift accent hue forward" },
          { combo: "Shift + H", description: "Shift accent hue backward" },
        ],
      },
      {
        title: "History explorer",
        shortcuts: [
          { combo: "[", description: "Step backward in history" },
          { combo: "]", description: "Step forward in history" },
          { combo: "Arrow Keys", description: "Navigate snapshots" },
        ],
      },
    ],
    []
  );

  useKeyboardShortcuts({
    "?": () => setShortcutOverlayOpen((open) => !open),
    "shift+/": () => setShortcutOverlayOpen((open) => !open),
    escape: () => setShortcutOverlayOpen(false),
    r: () => refresh(),
    h: () => setHue((value) => (value + 10) % 360),
    "shift+h": () => setHue((value) => (value - 10 + 360) % 360),
    "[": () => stepHistory(-1),
    "]": () => stepHistory(1),
    arrowleft: () => stepHistory(-1),
    arrowright: () => stepHistory(1),
    "ctrl+.": () => setAdminOpen((value) => !value),
    "meta+.": () => setAdminOpen((value) => !value),
  });

  const metricsView = useMemo(() => activeSnapshot, [activeSnapshot]);

  const renderErrors = () => {
    if (metricsView.recentErrors.length === 0) {
      return React.createElement("p", null, "No errors reported in the last 15 minutes.");
    }
    return React.createElement(
      "ul",
      null,
      metricsView.recentErrors.map((error) =>
        React.createElement(
          "li",
          { key: error.id },
          React.createElement("strong", null, error.code),
          " — ",
          error.message,
          " (",
          new Date(error.timestamp).toLocaleTimeString(),
          ")"
        )
      )
    );
  };

  return React.createElement(
    "div",
    { className: "app-shell" },
    React.createElement(
      "header",
      { className: "app-header" },
      React.createElement(
        "div",
        { className: "branding" },
        React.createElement("img", { src: logoUrl, alt: "AI Ticket" }),
        React.createElement("h1", null, "AI Ticket Operations")
      ),
      React.createElement(
        "div",
        { className: "controls-bar" },
        React.createElement(HueSlider, { value: hue, onChange: setHue }),
        React.createElement("span", { className: "shortcuts-hint" }, "Press ⇧ / for shortcuts")
      )
    ),
    React.createElement(
      "main",
      { className: "app-content" },
      React.createElement(
        "section",
        { className: "panel", "aria-label": "Key metrics" },
        React.createElement("h2", null, "Realtime Metrics"),
        React.createElement(MetricsPanel, { metrics: metricsView })
      ),
      React.createElement(
        "section",
        { className: "panel", "aria-label": "System status" },
        React.createElement("h2", null, "System Status"),
        React.createElement(StatusGrid, { statuses: metricsView.statusPanels, lastUpdated: metricsView.timestamp })
      ),
      React.createElement(
        "section",
        { className: "panel", "aria-label": "Error activity" },
        React.createElement("h2", null, "Recent Errors"),
        renderErrors()
      ),
      React.createElement(
        "section",
        { className: "panel", "aria-label": "History explorer" },
        React.createElement("h2", null, "History Explorer"),
        React.createElement(HistoryExplorer, {
          history,
          activeSnapshot,
          selectedTimestamp,
          onSelectTimestamp: selectTimestamp,
          onStepBackward: () => stepHistory(-1),
          onStepForward: () => stepHistory(1),
          isLive,
          followLive,
          onFollowLiveChange: setFollowLive,
          onJumpToLive: jumpToLive,
        })
      ),
      React.createElement(
        "section",
        { className: "panel", "aria-label": "Admin panel", "data-testid": "admin-panel-section" },
        React.createElement("h2", null, "Admin Panel"),
        React.createElement(AdminPanel, {
          open: adminOpen,
          onToggle: setAdminOpen,
          theme,
          onThemeChange: setTheme,
          streamPaused,
          onStreamPausedChange: setStreamPaused,
          followLive,
          onFollowLiveChange: setFollowLive,
          reducedMotion,
          onReducedMotionChange: setReducedMotion,
          onRefresh: refresh,
        })
      )
    ),
    shortcutOverlayOpen
      ? React.createElement(ShortcutOverlay, {
          sections: shortcutSections,
          onClose: () => setShortcutOverlayOpen(false),
        })
      : null
  );
};

export default App;
