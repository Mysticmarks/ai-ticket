...
def _run_with_uvicorn(options: Mapping[str, object]) -> None:
    import uvicorn

    run_kwargs = {key: value for key, value in options.items() if value is not None}
    uvicorn.run("ai_ticket.server:app", **run_kwargs)
...
def _serve_command(args: argparse.Namespace, ctx: _CLIContext) -> int:
    ...
    summary_parts = [
        f"Uvicorn on {bind}",
        f"workers={args.workers if not args.reload else 1}",
        f"limit={args.limit_concurrency}",
        f"backlog={args.backlog}",
    ]
    if args.reload:
        summary_parts.append("reload")
    _print_panel("Server", " — ".join(summary_parts), ctx.accent)

    try:
        _run_with_uvicorn(config)
        ...
        _print_panel(
            "Server",
            "Uvicorn is required for production serving. Install it via pip.",
            "red",
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    serve_parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of Uvicorn worker processes to spawn (ignored when reload is enabled).",
    )
    serve_parser.add_argument(
        "--keepalive",
        type=int,
    ...
    serve_parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Maximum number of concurrent connections handled per worker.",
    )
    serve_parser.add_argument(
        "--backlog",
        type=int,
        default=2048,
        help="Maximum number of pending connections in the socket backlog.",
    )
    serve_parser.add_argument(
        "--reload",
        action="store_true",
