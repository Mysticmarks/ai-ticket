...
def on_event(event_data: dict):
    ...
    prompt_text = None
    messages_found_without_user_prompt = False
    try:
        ...
        if isinstance(inner_data, dict):
            if "messages" in inner_data and isinstance(inner_data["messages"], list):
                ...
                if user_prompts:
                    ...
                    messages_found_without_user_prompt = True
    ...
    if messages_found_without_user_prompt:
        logging.error("Prompt extraction failed: No user messages present in 'messages'.")
        return {
    ...
    print(f"Extracted prompt: '{prompt_text}'")  # pragma: no cover

    result = get_kobold_completion(prompt=prompt_text)

    if "completion" in result:
        print(f"KoboldCPP completion: {result['completion']}")  # pragma: no cover
        return {"completion": result["completion"]}

    error = result.get("error", "api_unknown_error")
    details = result.get(
        "details",
        "Failed to get completion from KoboldCPP.",
    )
    print(f"Failed to get completion from KoboldCPP. Error: {error}. Details: {details}")  # pragma: no cover
